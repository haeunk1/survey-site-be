...
public class RefreshToken {
    ...
    private void validTokenInnerMember() {
        if (this.member == null) {
            throw new ServeyException(ErrorCode.REFRESH_TOKEN_NOT_HAVE_MEMBER);
        }
    }
    ...
    private void validTokenExpiryDateExist() {
        if (ObjectUtils.isEmpty(this.expiryDate)) {
            throw new ServeyException(ErrorCode.REFRESH_TOKEN_EXPIRATION_DATE_NOT_FOUND);
        }
    }
    ...
    private void validTokenExpiryDateExpiration() {
        if (this.expiryDate.isBefore(LocalDateTime.now())) {
            throw new ServeyException(ErrorCode.REFRESH_TOKEN_EXPIRED);
        }
    }
