...
public class Member {
    ...
    public void changeEmail(String email) {
        if (ObjectUtils.isEmpty(email)) {
            throw new SurveyException(ErrorCode.CHANGE_EMAIL_VALUE_NOT_FOUND);
        }
        this.email = email;
    }
    ...
    public void changeMemberInsideJwt(Jwt jwt) {
        if (!ObjectUtils.isEmpty(this.jwt)) {
            throw new SurveyException(ErrorCode.MEMBER_JWT_ALREADY_EXIST);
        }
        this.jwt = jwt;
    }
    ...
    public void validSignUpMemberData() {
        ...
        if (email == null || password == null || name == null) {
            throw new SurveyException(ErrorCode.MEMBER_REQUIRED_VALUE);
        }
    }
    ...
    public boolean isSameEmail(String email) {
        if (ObjectUtils.isEmpty(email)) {
            throw new SurveyException(ErrorCode.MEMBER_EMAIL_PARAM_NOT_FOUND);
        }

        if (ObjectUtils.isEmpty(this.email)) {
            throw new SurveyException(ErrorCode.MEMBER_INNER_EMAIL_NOT_FOUND);
        }

        if (isNotSameEmail(email)) {
            throw new SurveyException(ErrorCode.MEMBER_EMAIL_NOT_MATCH);
        }
